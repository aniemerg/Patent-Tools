...
def populate(inputFile):
    ...
    with con:
        cur = con.cursor()
        cur.execute(
            "CREATE TABLE IF NOT EXISTS USPatents(patentNumber VARCHAR(8), abstract TEXT, inventor TEXT, \
        ...
        cur.execute("CREATE TABLE IF NOT EXISTS \
             FurtherUSClass(patentNumber VARCHAR(8), furtherUSClass TEXT)");

        cur.execute("CREATE TABLE IF NOT EXISTS \
             FieldOfResearch(patentNumber INT, class TEXT)");

        cur.execute("CREATE TABLE IF NOT EXISTS \
            PublicationReference(patentNumber VARCHAR(8), country TEXT, kind TEXT, name TEXT, date BIGINT)");

        cur.execute("CREATE TABLE IF NOT EXISTS \
            ApplicationReference(patentNumber VARCHAR(8), docNumber TEXT, country TEXT, date BIGINT)");

        cur.execute("CREATE TABLE IF NOT EXISTS \
            ReferPatcit(patentNumber INT, docNumber TEXT, country TEXT, kind TEXT, category TEXT, name TEXT, date BIGINT)");

        cur.execute("CREATE TABLE IF NOT EXISTS \
            ReferNplcit(patentNumber VARCHAR(8), value TEXT, category TEXT)");
        ...
        try:
            if _t.find('</us-patent-grant>') > -1:
                ...
                #patent number
                    ...
                    #field of research
                    for rs in n.iter("us-field-of-classification-search"):
                    ...
                    #International Class, get all the information and put them in the string

                    for ic in n.iter("classifications-ipcr"):
                        date = ''
                        section = ''
                        cl = ''
                        mainGroup = ''
                        subgroup = ''
                        subclass = ''
                        for _ic in ic.iter("classification-ipcr"):
                            for d in _ic.iterfind("ipc-version-indicator/date"):
                                if d.text: date = d.text
                            for tl in _ic.iter("section"):
                                if tl.text: section = tl.text
                            for tl in _ic.iter("class"):
                                if tl.text: cl = tl.text
                            for tl in _ic.iter("subclass"):
                                if tl.text: subclass = tl.text
                            for tl in _ic.iter("main-group"):
                                if tl.text: mainGroup = tl.text
                            for tl in _ic.iter("subgroup"):
                                if tl.text: subgroup = tl.text
                            with con:
                                cur = con.cursor()
                                cur.execute(
                                    "INSERT INTO InternationalClass(patentNumber, interClass) VALUES(%s, %s)"
                                    , (patentNumber, section + cl + subclass +" " + mainGroup + "/" + subgroup + "(" + date + ")"))
                    ...
                    if cl is not None:
                        _cl = tostring(cl)
                        claim = convertToHTMLView(_cl.encode('UTF-8'))
                    ...
                    if des is not None:
                        _des = tostring(des)
                        description = convertToHTMLView(_des.encode('UTF-8'))
